...
\color{blue}
\par
To implement such a scheduler, I used the SRT scheduler with two clients as a starting point. The key idea is to add a state variable to the $\Pname{Scheduler}$ module that keeps track of the last served job. Since there are only two clients in this scenario, a simple boolean flag (e.g., $\Pname{just\_served1}$) is sufficient. 

First, consider the scenario where both $\Pname{job1}$ and $\Pname{job2}$ are waiting to be served:
\begin{itemize}
    \item We allow the $\Pname{serve1}$ operation if client 1 was not just served (i.e., the flag is set to \emph{false}).
    \item We allow the $\Pname{serve2}$ operation if client 1 was just served (i.e., the flag is set to \emph{true}).
\end{itemize}
Naturally, during these transitions, we toggle the $\Pname{just\_served1}$ flag accordingly (setting it to \emph{true} after serving client 1, and \emph{false} after serving client 2).

Next, we can consider the case where only one of the two jobs is waiting. In this situation, we always serve the waiting job without checking the flag's current state. However, we must still update $\Pname{just\_served1}$ just as we did in the previous transitions, ensuring that the strict round-robin alternation resumes correctly as soon as both jobs are waiting again.
\color{black}
\fi
%
...
\color{blue}
\par
No changes are needed for the $\Pname{create}$ and $\Pname{finish}$ commands, since the flag is simply initialized to \emph{false} in the scheduler and is exclusively modified during the $\Pname{serve}$ commands.
\color{red} 
\par UPLOAD REQUIRED: the new prism model \texttt{A02P.1.b.prism}.\color{black}
\color{black}
\fi
%
...
\color{blue}
\par
There are \textbf{98} reachable states. See Figure~\ref{fig:RR_reachable_states}.
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/A02/RR_reachable_states.png}
    \caption{Number of reachable states in RR scheduler.}
    \label{fig:RR_reachable_states}
\end{figure}
\color{black}
\fi
%
...
\color{blue}
\par
There is no need to edit the previous properties. In particular, we will reuse the properties from the SRT model to verify that if a client has an active job, it is correctly tracked in the queue, alongside the global properties to check for starvation. Since we need to verify this for both clients, we have a total of four properties:

\begin{itemize}
\item \texttt{A [ G (state1=1 => job1=true) ]}: verifies the queueing logic for client 1 (see Figure~\ref{fig:RR_queue_client1}).
\item \texttt{A [ G (state2=1 => job2=true) ]}: verifies the queueing logic for client 2 (see Figure~\ref{fig:RR_queue_client2}).
\item \texttt{P>=1 [ G task1>0 => P>=1 [ F task1=0 ] ]}: verifies the absence of starvation for client 1 (see Figure~\ref{fig:RR_starvation_client1}).
\item \texttt{P>=1 [ G task2>0 => P>=1 [ F task2=0 ] ]}: verifies the absence of starvation for client 2 (see Figure~\ref{fig:RR_starvation_client2}).
\end{itemize}

The evaluation results for these properties on the model are shown below.

\begin{figure}[h!]
    \centering
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/A02/prop_serve_client1.png}
        \caption{Queueing property for client 1.}
        \label{fig:RR_queue_client1}
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/A02/prop_serve_client2.png}
        \caption{Queueing property for client 2.}
        \label{fig:RR_queue_client2}
    \end{minipage}
    
    \vspace{0.5cm} % Spazio verticale tra le due righe di immagini
    
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/A02/prop_starvation_client1.png}
        \caption{Starvation property for client 1.}
        \label{fig:RR_starvation_client1}
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/A02/prop_starvation_client2.png}
        \caption{Starvation property for client 2.}
        \label{fig:RR_starvation_client2}
    \end{minipage}
\end{figure}
...
\color{blue}
\par
None of the checks failed: if either client 1 or client 2 has an active job, it is correctly maintained in the queue. Furthermore, thanks to the round-robin policy, both jobs are guaranteed to eventually complete without starvation, meaning all verified properties hold true.
\color{black}
\fi
%
\end{enumerate}
...
\begin{figure}[h]
\begin{center}
\includegraphics[width=.75\textwidth]{figures/A02/bisimulation.pdf}
\end{center}
